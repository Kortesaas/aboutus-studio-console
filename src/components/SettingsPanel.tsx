...
import type { ConnectionStatus, EntityMappings } from '../services/smartHome'
...
export function SettingsPanel() {
  ...
  useEffect(() => {
    ...
    setDeviceResult('')
  }, [configuredUrl, dashboardConfig, entityMappings, settingsOpen])
    ...
    <div className="settings-overlay" role="dialog" aria-modal="true" aria-labelledby="settings-title">
      <div className="settings-shell">
        ...
        <div className="settings-content">
          <section className="settings-section" aria-labelledby="settings-ha-title">
          ...
          <form className="settings-section" onSubmit={(event) => void handleDeviceSave(event)}>
            <div className="settings-section-heading">
              <div>
            ...
            <div className="device-settings-grid">
              {configuredDevices.map((definition) => {
                ...
                const options = [...availableEntities].sort((a, b) => {
                ...
                return (
                  <article className={`device-settings-row ${slot.enabled ? '' : 'is-disabled'}`} key={definition.id}>
                    ...
                    <label>
                      ...
                      <select
                        value={slot.roomId}
                        onChange={(event) => updateDeviceDraft(setDraftConfig, definition.id, { roomId: event.target.value as 'studio1' | 'studio2' })}
                      >
            ...
            <div className="group-settings">
              <div>
                ...
                <h2>Groups</h2>
                <p>Room assignment determines Studio groups. Everything contains every enabled slot.</p>
              </div>
              {(['studio1', 'studio2', 'everything'] as const).map((groupId) => (
                <label key={groupId}>
                  <span>{groupId === 'studio1' ? 'Studio 1 label' : groupId === 'studio2' ? 'Studio 2 label' : 'Everything label'}</span>
                  <input
                    type="text"
            ...
            <div className="settings-form-actions">
              ...
              <button className="settings-primary" type="submit" disabled={isSavingDevices}>
                {isSavingDevices ? 'Saving…' : 'Save devices & groups'}
              </button>
            </div>
          </form>
